  ...
  return (
    <>
      ...
      <main className="relative min-h-screen px-6 py-16 max-w-prose mx-auto" style={{ zIndex: 1 }}>
        <Link
          href="/"
          className="section-title inline-flex items-center gap-2 mb-10 hover:text-accent-vol3 transition-colors no-underline group"
        >
          <span className="group-hover:-translate-x-1 transition-transform">←</span> Back
        </Link>

        <p className="font-mono text-xs tracking-[0.3em] text-accent-vol3 uppercase mb-2">Volume III</p>
        <h1 className="font-sans text-4xl md:text-5xl font-semibold text-text mb-4">
          The Bigger World
        </h1>
        <p className="font-serif text-muted text-lg mb-8">
          A guide for young explorers of an amazing universe
        </p>
        <div className="h-px bg-border mb-10" />

        <div className="prose-warm panel p-6 mb-10 border-accent-vol3/30">
          <p className="text-lg">
            Did you know that the universe is much bigger, stranger, and more wonderful than most
            people were ever taught? This book is your guide to those bigger truths — written for
            curious young explorers who want to understand what is going on in the world right now,
            and who they might become in response to it.
          </p>
          <p className="text-sm mt-4">
            The three great shifts of our time — the rise of artificial intelligence, the opening of
            government files on unexplained sky phenomena, and discoveries about consciousness — are
            shaping the world you will inherit. Each chapter offers tools, stories, and exercises to
            help you think clearly, feel grounded, and act wisely.
          </p>
        </div>
        ...
        <nav className="mt-10 space-y-3" aria-label="Table of Contents">
          {chapters.map((ch) => (
            <Link
              ...
              className="group flex gap-4 rounded-lg border border-border bg-panel p-4 hover:border-accent-vol3/50 transition-colors no-underline"
            >
              <span className="font-mono text-xs text-accent-vol3 uppercase tracking-wide shrink-0 w-20 pt-0.5">
                Ch. {ch.number}
              </span>
              <div>
                <p className="font-sans font-medium text-text group-hover:text-accent-vol3 transition-colors">
                  {ch.title}
                </p>
                <p className="text-sm text-muted mt-1 font-serif leading-snug">{ch.description}</p>
              </div>
            </Link>
          ))}
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm text-muted mt-1 font-serif leading-snug">
                References, further reading, and resources organised by chapter for young explorers
                and their families.
              </p>
            </div>
          </Link>
        ...
        <div className="mt-16 pt-8 border-t border-border">
          <Link
            ...
            className="inline-flex items-center gap-2 font-sans text-accent-vol3 hover:underline underline-offset-4 no-underline"
          >
